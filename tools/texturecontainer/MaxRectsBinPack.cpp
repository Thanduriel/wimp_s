...
Rect MaxRectsBinPack::Insert(int width, int height, FreeRectChoiceHeuristic method)
{
	...
	switch(method)
	{
	case FreeRectChoiceHeuristic::RectBestShortSideFit: newNode = FindPositionForNewNodeBestShortSideFit(width, height, score1, score2); break;
	case FreeRectChoiceHeuristic::RectBottomLeftRule: newNode = FindPositionForNewNodeBottomLeft(width, height, score1, score2); break;
	case FreeRectChoiceHeuristic::RectContactPointRule: newNode = FindPositionForNewNodeContactPoint(width, height, score1); break;
	case FreeRectChoiceHeuristic::RectBestLongSideFit: newNode = FindPositionForNewNodeBestLongSideFit(width, height, score2, score1); break;
	case FreeRectChoiceHeuristic::RectBestAreaFit: newNode = FindPositionForNewNodeBestAreaFit(width, height, score1, score2); break;
	}
...
bool MaxRectsBinPack::Insert(std::vector<std::pair<RectSize,size_t>> rects, std::vector<Rect> &dst, FreeRectChoiceHeuristic method)
{
	...
	while(rects.size() > 0)
	{
		...
		for(int i = 0; i < (int)rects.size(); ++i)
		{
			int score1;
			int score2;
			Rect newNode = ScoreRect(rects[i].first.width, rects[i].first.height, method, score1, score2);
		...
		PlaceRect(bestNode, rects[bestRectIndex].second);
		rects.erase(rects.begin() + bestRectIndex);
	}
	return true;
}

void MaxRectsBinPack::PlaceRect(const Rect &node, size_t index)
{
	size_t numRectanglesToProcess = freeRectangles.size();
	for(size_t i = 0; i < numRectanglesToProcess; ++i)
	{
...
Rect MaxRectsBinPack::ScoreRect(int width, int height, FreeRectChoiceHeuristic method, int &score1, int &score2) const
{
	...
	case FreeRectChoiceHeuristic::RectBestShortSideFit: newNode = FindPositionForNewNodeBestShortSideFit(width, height, score1, score2); break;
	case FreeRectChoiceHeuristic::RectBottomLeftRule: newNode = FindPositionForNewNodeBottomLeft(width, height, score1, score2); break;
	case FreeRectChoiceHeuristic::RectContactPointRule: newNode = FindPositionForNewNodeContactPoint(width, height, score1);
