...
namespace Graphic {
	...
	static void ActivateOpenGLDebugging()
	{
		...
		GL_CALL(glDebugMessageCallback, reinterpret_cast<GLDEBUGPROC>(&DebugErrorCallback), nullptr);
	}
	...
	void Device::BindFramebuffer(const Framebuffer* _framebuffer, bool _autoViewportSet)
	{
		...
		// setup draw buffers
		if(previousColorTargetCount != currentColorTargetCount && g_Device.m_BoundFrameBuffer)
		{
			...
			GL_CALL(glDrawBuffers, currentColorTargetCount, drawBuffers);
		}
	}
