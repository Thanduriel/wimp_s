...
namespace GameStates {
	...
	SettingsHud::SettingsHud()
	{
		...
		m_aimAssistButton = &CreateScreenElement<Button>("menuBtn", PixelOffset(0, 300), Vec2(0.f), DefinitionPoint::BotMid, Anchor(DefinitionPoint::MidMid, this), "aim assist: OFF");
		...
		m_resolutionButton = &CreateScreenElement<Button>("menuBtn", PixelOffset(0, -40), Vec2(0.f), DefinitionPoint::TopMid, Anchor(DefinitionPoint::BotLeft, m_mouseSensitivity), "1920x1080");
		m_resolutionButton->Scale(Vec2(0.5f, 0.5f));
		m_fullScreenButton = &CreateScreenElement<Button>("menuBtn", PixelOffset(0, -20), Vec2(0.f), DefinitionPoint::TopMid, Anchor(DefinitionPoint::BotMid, m_resolutionButton), "fullscreen: OFF");
		...
		TextRender& label2 = CreateScreenElement<TextRender>(Vec2(0.f), Anchor(DefP::TopLeft, m_frameCountTarget), nullptr, "target frame rate: ");
		label2.SetDefaultSize(0.6f);
		label2.SetPosition(Vec2(-label2.GetRectangle().x, 0.f));

		TextRender& note = CreateScreenElement<TextRender>(PixelOffset(0, 40), Anchor(DefP::TopLeft, m_applyButton), nullptr, "changes to video settings require a restart");
		note.SetDefaultSize(0.3f);
	}
}