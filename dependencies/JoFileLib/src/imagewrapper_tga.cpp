...
#pragma pack( pop )
	...
	void ImageWrapper::ReadTGA( const IFile& _file )
	{
		...
		std::unique_ptr<uint8_t[]> colorMap;
		if( header.colorMapType == 1 )
		{
			...
			colorMap = std::make_unique<uint8_t[]>(colorMapSize);

			_file.Seek( header.colorMap.offset );
			_file.Read( colorMapSize, colorMap.get() );
			...
			while( x < m_width )
			{
				...
				ReadColor:
					...
					if( colorMap ) {
						...
						color = *(uint32_t*)(colorMap.get() + color * ((header.colorMap.depth + 1)/8));
					}
				}
